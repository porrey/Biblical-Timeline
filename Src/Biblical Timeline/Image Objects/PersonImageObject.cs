...
namespace Biblical.Timeline
{
	internal class PersonImageObject(BiblicalEvent biblicalEvent, PageDefinition pageDefinition, TimelineParameters parameters) : ImageObjectTemplate(biblicalEvent, pageDefinition, parameters)
	{
		protected override Task OnDraw(Graphics g)
		{
			...
			// Draw the rectangle.
			//
			g.FillRectangle(this.PageDefinition.Theme.BarDarkBackgroundBrush, this.Rectangle);
			g.DrawRectangle(this.PageDefinition.Theme.BarDarkBorderPen, this.Rectangle);
			...
			g.DrawString(text, this.PageDefinition.Theme.NameFont, this.PageDefinition.Theme.BarDarkTextBrush, new PointF(textLeft, top1));
			...
			if (this.BiblicalEvent.DisplayEventLength)
			{
				...
				g.DrawString(lengthText, this.PageDefinition.Theme.LengthFont, this.PageDefinition.Theme.BarDarkTextBrush, new PointF(textLeft + size1.Width - 10, top1 + ((size1.Height - size2.Height) / 2)));
			}

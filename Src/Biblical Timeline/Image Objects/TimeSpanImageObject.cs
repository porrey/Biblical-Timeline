...
namespace Biblical.Timeline
{
	internal class TimeSpanImageObject(BiblicalEvent biblicalEvent, PageDefinition pageDefinition, TimelineParameters parameters) : ImageObjectTemplate(biblicalEvent, pageDefinition, parameters)
	{
		protected override Task OnDraw(Graphics g)
		{
			...
			// Draw the rectangle.
			//
			g.FillRectangle(this.PageDefinition.Theme.BarLightBackgroundBrush, this.Rectangle);
			g.DrawRectangle(this.PageDefinition.Theme.BarLightBorderPen, this.Rectangle);
			...
			g.DrawString(text, this.PageDefinition.Theme.NameFont, this.PageDefinition.Theme.BarLightTextBrush, new PointF(textLeft, top1));
			...
			if (this.BiblicalEvent.DisplayEventLength)
			{
				...
				g.DrawString(lengthText, this.PageDefinition.Theme.LengthFont, this.PageDefinition.Theme.BarLightTextBrush, new PointF(textLeft + size1.Width - 10, top1 + ((size1.Height - size2.Height) / 2)));
			}
